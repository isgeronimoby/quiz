...
var ddimagepanner = {
	...
	magnifyicons: [imagePannerMagnifyPath, imagePannerMagnify2Path, 19, 18], //set path to zoom in/out images, plus their dimensions, paths initialized in ScriptsBlock view
	maxzoom: 4, //set maximum zoom level (from 1x)

	init: function ($, $img, options) {
		var s = options;
		
		var imgHeight = s.wrappersize[1] > $img.height() ? s.wrappersize[1] : $img.height();
		var imgWidth = s.wrappersize[1] > $img.height() ? parseInt($img.width() * s.wrappersize[1] / $img.height()) : $img.width();
		
		s.imagesize = s.imagesize === undefined ? [imgWidth, imgHeight] : s.imagesize;
		s.oimagesize = [imgWidth, imgHeight]//[$img.width(), $img.height()] //always remember image's original size
		s.pos = (s.pos == "center") ? [-(s.imagesize[0] / 2 - s.wrappersize[0] / 2), -(s.imagesize[1] / 2 - s.wrappersize[1] / 2)] : s.pos //initial coords of image
...
jQuery(document).ready(function ($) { //By default look for DIVs with class="pancontainer"
	...
	$pancontainer.each(function () {
		...
		if ($this.attr("data-width") && $this.attr("data-height")) {
			imageSize = [parseInt($this.attr('data-width')), parseInt($this.attr('data-height'))];
		}
